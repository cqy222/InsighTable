...
from htable.visEncoding import VisEncoding
...
def cell_emb(x):
    ...
    if cell_dict.__contains__(x):
        return cell_dict[x]
    y = np.array([get_fea1(x), get_fea2(x), get_fea3(x), get_fea4(x)])
    cell_dict[x] = y
    # print('bbbb', x, y[:4])
    return y
...
class EvaENV(gym.Env):
    ...
    def get_obs(self):
        ...
        if self.use_tow_stage:
            ...
            if self.use_gnn:
                ...
                if self.stage == 1:
                    x[:,bert_embshape:bert_embshape+32] = -(self.cur_insight_type+1)/self._insight_num
                    x[:,bert_embshape+32:] = -1
                    for index in self.select_row:
                        if index in self.H_set:
                            x[self.H_map[index]][bert_embshape+32:] = 2
                    for index in self.select_col:
                        if index in self.H_set:
                            x[self.H_map[index]][bert_embshape+32:] = 1
                ...
                # print('xxx', x.shape, edge_index.shape)
                ...
                # print('table_datas', table_datas.shape)
                # print('z', z.shape)
                ...
                # # print('x_order', x_order.shape)
                # # print('y_order', y_order.shape)
                ...
                datas = np.concatenate((np.array([self.stage, self.stage0_size/100., (self.cur_insight_type+1)/self._insight_num, \
                        self.table.shape[0], self.table.shape[1], len(self.H_set), self.max_table_width, self.max_table_height]), z))
                ...
                obs = np.concatenate((np.array([self.stage, self.stage0_size/100., self.cur_insight_type]), table_fea.flatten()))
    ...
    def step(self, action):
        ...
        try:
            ...
            reward = 0
            area_ratio = Evenness_index = insight_ratio = 0
            done = True
            print('Exception', ex)
            raise NotImplementedError
