...
class EvaENV(gym.Env):
    def __init__(self, data_name=None, eval=False):
        if data_name == None:
        ...
        if self.data_name == 'Console5':
            ...
            if os.path.exists('./htable/console_bert_dict.npy'):
                self.bert_dict = np.load('./htable/console_bert_dict.npy', allow_pickle=True).item()
            else:
                self.bert_dict = {}
            df = pd.read_excel('./htable/public/Console Sales.xlsx', header=[0, 1], index_col=[0, 1, 2])#.sort_index(axis=0).sort_index(axis=1)
            self.dataset.append(df)
            ...
            self.bert_dict = np.load('./htable/income_bert_dict.npy', allow_pickle=True).item()
    ...
    def reset(self):
        ...
        start_x = np.random.randint(self.table.shape[0]) // 5 * 5
        start_y = np.random.randint(self.table.shape[1]) // 5 * 5
    ...
    def bert_emb(self, x):
        ...
        if self.extraction == None:
            self.extraction = pipeline("feature-extraction", device=0, model="bert-base-chinese", tokenizer="bert-base-chinese")
        y = np.array(self.extraction(x)[0][0])
        self.bert_dict[x] = y
        print('new_bert_dict', x, y[:4])
        np.save('./htable/console_bert_dict.npy', self.bert_dict)
    ...
    def step(self, action):
        ...
        # info = {"reward":self.episodeR, "insight_ratio":0, "area_ratio":0, "Evenness_index":0, "terminated":False}
