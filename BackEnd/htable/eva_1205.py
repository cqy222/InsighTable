...
# bert_dict = np.load('bert_dict.npy', allow_pickle=True).item()
...
class EvaENV(gym.Env):
    ...
    def reset(self):
        ...
        if self.use_gnn:
            ...
            # print('self.H_set', self.H_set)
            self.embeddings = np.array([bert_emb(header) for header in self.H_set])
    ...
    def get_obs(self):
        ...
        if self.use_tow_stage:
            ...
            if self.use_gnn:
                ...
                x = np.zeros([len(self.H_set), bert_embshape + 32])
                x[:,0:bert_embshape] = self.embeddings[:,0:bert_embshape]
                if self.stage == 1:
                    x[:,bert_embshape:] = -1
                    for index in self.select_row:
                        if index in self.H_set:
                            x[self.H_map[index]][bert_embshape:] = 2
                    for index in self.select_col:
                        if index in self.H_set:
                            x[self.H_map[index]][bert_embshape:] = 1
                ...
                z = self.mask.values.flatten().copy() *1./self._insight_num
    ...
    def step(self, action):
        ...
        if done or self.steps >= self._all_step:
            ...
            info = {"reward":self.episodeR, "insight_ratio":insight_ratio, "area_ratio":area_ratio, "Evenness_index":Evenness_index, "terminated":True}
