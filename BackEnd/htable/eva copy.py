...
# ------------- bert emb -------------
bert_dict = {}
extraction = pipeline("feature-extraction", framework='pt')
bert_embshape = 64
def bert_emb(x):
...
class EvaENV(object):
    def __init__(self, tuta_tools):
        ...
        self._transformation_step = int(200 * ratio)
        self._transformation_step = 0
        self._insight_step = (200 - self._transformation_step) // len(self._insight_type_single + self._insight_type_multiple)
        ...
        #     self._transformation_step = 50
            ...
            self.path = './rlpyt/envs/htable/hmt'
    ...
    def get_obs(self):
        ...
        if self.use_tow_stage:
            ...
            if self.use_gnn:
                ...
                # x[:,0:bert_embshape] = self.embeddings
                # if self.stage == 1:
                ...
                # -------------------------------------------
                x = np.zeros([len(self.H_set), 32 + 32 + 32])
                x[:,0:32] = self.embeddings[:,:32]
                x_list = []
                y_list = []
                if self.stage == 1:
                    x[:,32:32+32] = -(self.cur_insight_type+1)/self._insight_num
                    x[:,32+32:] = -1
                    for index in self.select_row:
                        if index in self.H_set:
                            x[self.H_map[index]][32+32:] = 2
                            x_list.append(index)
                    for index in self.select_col:
                        if index in self.H_set:
                            x[self.H_map[index]][32+32:] = 1
                            y_list.append(index)
                ...
                z = np.apply_along_axis(bert_emb, -1, z[:, :, np.newaxis])
                # print('zzz', z.shape, z)
    ...
    def step(self, action):
        try:
            ...
            if self.use_tow_stage:
                if self.stage == 0 and action >= self.stage0_size or self.stage == 1 and 0 < action < self.stage0_size:
                    print('error action', self.steps, self.stage, action, self.stage0_size)
                    raise NotImplementedError
            ...
            # reward = 0
            # if self.select_row == self.table.index[5] or self.select_col == self.table.columns[5]:
            #     reward = 1
        except Exception as ex:
            reward = 0
        ...
        if done or self.steps > self.episode_step:
            ...
            if self.log_enc:
                path = 'test_results/0629_17_13_52/ppo/Console_10000' 
