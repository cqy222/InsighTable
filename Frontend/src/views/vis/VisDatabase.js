...
VisDatabase.prototype.GetGroupID = function (id) {
  if (this.database[id] == null) return null
  else return this.database[id]["group_id"];
};
...
VisDatabase.prototype.DeleteGroup = function (group_id) {
  if (group_id in this.group) {
    for (let i = 0, len = this.group[group_id].length; i < len; i++) {
      let memberID = this.group[group_id][i];
      // console.error("DeleteGroupDeleteGroup", group_id, memberID);
      this.RemoveCanvas(memberID);
      this.bus.$emit("remove-ins-marker", memberID);
    }
  }
  else {
    this.RemoveCanvas(group_id);
  }
};
VisDatabase.prototype.HiddenGroup = function (group_id) {
  if (group_id in this.group) {
    for (let i = 0, len = this.group[group_id].length; i < len; i++) {
      let memberID = this.group[group_id][i];
      this.HiddenCanvas(memberID);
    }
  }
  else {
    this.HiddenCanvas(group_id);
  }
};
VisDatabase.prototype.DisplayGroup = function (group_id) {
  if (group_id in this.group) {
    for (let i = 0, len = this.group[group_id].length; i < len; i++) {
      let memberID = this.group[group_id][i];
      this.DisplayCanvas(memberID);
    }
  }
  else {
    this.DisplayCanvas(group_id);
  }
};
...
VisDatabase.prototype.AddGroupMember = function (group_id, id) {
  // console.error("AddGroupMemberAddGroupMember", group_id, id);
  ...
  if (this.database.hasOwnProperty(id)) {
    ...
    if (this.database[id].status == status.clear) {
      for (let i = 0; i < this.GetGroupMembers(id).length; i++) {
        ...
        // console.log("select-canvas otherID", otherID)
      }
      // console.log("select-canvas id", id)
    ...
    // console.error("databasedatabasedatabase", typeof this.database, this.database)
    for (const key in this.database) {
      if (Object.hasOwnProperty.call(this.database, key)) {
        if (key == id) {
          // console.log("Object.hasOwnProperty", key, id, this.database[id], this.database[id].hp_pos)
          if (this.database[id].hp_pos == undefined){
            console.error("alternative-find-undefined", this.database[id])
          }
          // console.log("alternative-find", this.database[id]);
          this.bus.$emit("alternative-find", this.database[id].hp_pos);
          break;
        }
      }
    }
...
VisDatabase.prototype.AddCloseButton = function (id) {
  ...
  button_box.addEventListener("click", () => {
    console.log("RemoveCanvasRemoveCanvas", id);
    // console.log("remove-all-insight-list-byid", this.database[id]);
    if (this.GetGroupMembers(id).length > 1) {
      ...
      console.log("groupCanvasgroupCanvas", this.database[id].group_id, id);
    }
    this.RemoveCanvas(id);
    // d3.selectAll("#ins-marker-" + fid).remove();
    this.bus.$emit("remove-ins-marker", id);
    this.bus.$emit("TableView-remove-id", id);
    // this.bus.$emit("remove-all-insight-list-byid", id);
  });
...
VisDatabase.prototype.ReconfigAllCanvas = function (
  ...
  // move col
  // console.log("RerenderCanvas-ReconfigAllCanvas");
...
VisDatabase.prototype.RerenderCanvas = function (
  ...
  console.log("RenderCanvas-RerenderCanvas");
...
VisDatabase.prototype.RemoveCanvas = function (id) {
  // console.error("RemoveCanvasRemoveCanvas", id);
  if (!!document.getElementById(id)) {
    // console.log('VisDatabase.RemoveCanvas', id)
    document.getElementById(id).remove();
    // console.error("RemoveCanvashhhhhhhave", id);
  }
  // else
    // console.error("RemoveCanvasnullllllllll", id);
...
VisDatabase.prototype.HiddenCanvas = function (id) {
  if (!!document.getElementById(id)) {
    // console.log('VisDatabase.RemoveCanvas', id)
...
VisDatabase.prototype.DisplayCanvas = function (id) {
  if (!!document.getElementById(id)) {
    // console.log('VisDatabase.RemoveCanvas', id)
...
VisDatabase.prototype.GetVegaLite = function (id, height, width) {
  // console.log("this.database[id]", this.database[id])
...
VisDatabase.prototype.isOverlap = function (height_num, width_num, x_num, y_num) {
  ...
  for (const id in this.database) {
    if (Object.hasOwnProperty.call(this.database, id) && !!document.getElementById(id) && document.getElementById(id).style.display != "" && document.getElementById(id).style.display != "none") {
      const metaData = this.database[id];
      if ((y_num > metaData.y && y_num < metaData.y + metaData.height) || (under > metaData.y && y_num < metaData.y + metaData.height)) {
        if ((x_num > metaData.x & x_num < metaData.x + metaData.width) || (right > metaData.x && x_num < metaData.x + metaData.width)) {
          console.log("isOverlap", this.database[id], typeof document.getElementById(id).style, document.getElementById(id)
...
VisDatabase.prototype.GenFig = function (
  ...
  if (hp_pos == undefined)
    console.error("alternative-find-GenFig", this.database, metaData);
  this.database[canvas_id] = metaData;
  this.database[canvas_id].type = "vega";
  this.database[canvas_id].hp_pos = hp_pos;
  // console.log("RenderCanvas-GenFig");
...
VisDatabase.prototype.GenUnit = function (
  ...
  console.log("RenderCanvas-GenUnit");
  this.RenderCanvas(canvas_id);
    return canvas_id;
  }
...
VisDatabase.prototype.DisableTableUnit = function (
  ...
  console.log("RenderCanvas-DisableTableUnit");
  this.RenderCanvas(canvas_id);
  return canvas_id;
};
VisDatabase.prototype.GenRecommendFigs = function (
  ...
  // console.error("AddGroupMember-GenRecommendFigs", groupID, currentID);
  // console.error("currentIDcurrentID", groupID, currentID);

  let currentTemplate = currentTemplate_obj;
  // if (recommendData_array.length > 0)
  //   console.log("GenRecommendFigs=============", currentTemplate);

  for (let i = 0, len = recommendData_array.length; i < len; i++) {
    let element = recommendData_array.at(i);
    let position = element.position;
    // console.log("recommendData_array", element.hp_pos_rec)
    if (!this.isOverlap(position.height, position.width, position.x, position.y)) {
      ...
      // console.error("currentIDcurrentID", groupID, id);
    }
    else
      console.log("-----every----isOverlap");
  }
  return groupID;
};

VisDatabase.prototype.ModifyGroupFigs = function (id, currentTemplate_obj) {
  // console.log("RerenderCanvas-ModifyGroupFigs");
...
VisDatabase.prototype.RenderCanvas = function (id, chartConfig=null) {
  // 1. get meta data
  // 2. tweak meta data
  // 3. gen chart
  // 4. append
  ...
  let height = this.database[id].height;
  let width = this.database[id].width;
  let x = this.database[id].x;
  let y = this.database[id].y;
  console.log("databasedatabase", this.database[id]);
  ...
  if (this.database[id].type == "unit") {
    ...
    if (canvas) {
      ...
      // add back ground
      // background.setAttribute("style", "fill:white; stroke:#8a8a8a; stroke-width:1px;");
      background.setAttribute("style", "fill:white");
      background.setAttribute("width", width);
      background.setAttribute("height", height);
      ...
      vegaEmbed("#chart-" + id, chartJson, {
        ...
        () => {
          // if (this.database[id].vegaTemplate.name == supportedTemplate.ANQN_Multi_Series_Line_Chart){
          //   console.error("#chart-ANQN_Multi_Series_Line_Chart", chartJson);
          // }
          ...
          if (this.database[id].vegaTemplate.name === supportedTemplate.Q2_Horizon_Graph){
            ...
            // console.log("Q2_Horizon_GraphQ2_Horizon_Graph", width, this.database[id].metaData.x.range, offset);
            content.setAttribute("transform", "translate(" + (-(offset / 2 + 0) * xScale) + "," + -7 + ") scale(" + (xScale) + ",1.5)");

            // content.setAttribute("transform", "translate(" + 0 + "," + 0 + ") scale(" + 1.5 + "," + 1.5 + ")");
          }
          else if (!!chartJson.mark && chartJson.mark.type == "area" && this.database[id].vegaTemplate.name != supportedTemplate.DensityPlot && this.database[id].vegaTemplate.name != supportedTemplate.Histogram_Area) {
            ...
            content.removeAttribute("transform");
            content.setAttribute("transform", "translate(" + 2 + "," + 2 + ")");
          }
          ...
          else if (this.database[id].vegaTemplate.name != supportedTemplate.DensityPlot && this.database[id].vegaTemplate.name != supportedTemplate.NQ_Parallel_Coordinate_Plot
                    && this.database[id].vegaTemplate.name != supportedTemplate.NQ_Box_Plot) {
            console.log("55555555555555555555");

            content.removeAttribute("transform");
            content.setAttribute("transform", "translate(" + 0 + "," + 4 + ")");
          }
...
//           // console.log("render JSON", chartJson);
//           let content = document.getElementById("chart-" + id);
//           if (this.database[id].vegaTemplate.name === supportedTemplate.Q2_Horizon_Graph || !!chartJson.mark && chartJson.mark.type == "area" && this.database[id].vegaTemplate.name != supportedTemplate.DensityPlot && this.database[id].vegaTemplate.name != supportedTemplate.Histogram_Area) {
//             content.removeAttribute("transform");
...
//             content.setAttribute("transform", "translate(" + (10) + "," + 10 + ")");
//           }
//           else if (this.database[id].vegaTemplate.name != supportedTemplate.DensityPlot) {
