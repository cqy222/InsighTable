...
export let supportedTemplate = {
    ...
    Histogram_Bar: "Histogram Bar",
    Histogram_Area: "Histogram Area"
}
...
export function GetTemplate(templateName_str, metaData_obj, visData_arr, direction) {
    ...
    switch (templateName_str) {
        case supportedTemplate.DensityPlot: {
            let density_template;
            // if (metaData_obj.x.range == 1 || metaData_obj.y.range == 1) {       // 只有一列数据
            picture = './templates/bar chart.png' // 懒得找了，就先用line
            vegaConfig = {
                mark: {"type":"area"},
                // mark: "bar",
                ...
                encoding: {
                    ...
                    color: {scale:{scheme:'set2'}}
                    // x: { "bin": true, field: "value"},
                    // y: { aggregate: "count"},
                },
            }
            density_template = new VegaTemplate(templateName_str, vegaConfig, selections_cell, picture);
            // }
        
            return density_template;
        }
        case supportedTemplate.Histogram_Area: {
            let density_template;
            // if (metaData_obj.x.range == 1 || metaData_obj.y.range == 1) {       // 只有一列数据
            picture = './templates/bar chart.png' // 懒得找了，就先用line
            vegaConfig = {
        ...
        case supportedTemplate.Histogram_Bar: {
            let density_template;
            // if (metaData_obj.x.range == 1 || metaData_obj.y.range == 1) {       // 只有一列数据
            picture = './templates/bar chart.png' // 懒得找了，就先用line
            vegaConfig = {
        ...
        case supportedTemplate.NQor2Q_Simple_Line_Chart:
            ...
            if (metaData_obj.x.range == 1 || metaData_obj.y.range == 1) {
                // console.log("linelineline1111", selections_cell.GetXSelection(0));
                // console.log("metaData_obj", metaData_obj)
                ...
                // console.log("linelinelineline", selections_horizon.GetQSelection(0), selections_horizon.GetQSelection(1));
                if (is_horizon) {
                    picture = './templates/line chart.png'
                    vegaConfig = {
                        mark: "line",
        ...
        case supportedTemplate.NQ_RadialPlot:
            if (is_horizon) {
                vegaConfig =
                {
                    ...
                    "mark": "arc",
                    "encoding": {
                        color: { field: selections_cell.GetXSelection(-1), type: "nominal", sort: selections_cell.GetSort(selections_cell.GetXSelection(-1)) ,scale:{scheme:'set2'}},
                        ...
                        "color": { "field": selections_horizon.GetXSelection(-1), "type": "nominal", scale:{scheme:'set2'} }
                    }
                }
            }
            else {
                vegaConfig =
                {
                    ...
                    "mark": "arc",
                    "encoding": {
                        color: { field: selections_cell.GetYSelection(-1), type: "nominal", sort: selections_cell.GetSort(selections_cell.GetYSelection(-1)), scale:{scheme:'set2'} },
                        theta: { aggregate: "sum", field: selections_cell.GetQSelection(0), "stack": true },
                        radius: {
                            aggregate: "sum", field: selections_cell.GetQSelection(0),
                            "scale": { "type": "linear", "zero": true, "rangeMin": 20, scale:{scheme:'set2'} }
                        },
                    }
                }
            }
        ...
        case supportedTemplate.ANQN_Multi_Series_Line_Chart:
            if (is_horizon) {
                vegaConfig = {
                    data: { values: visData_arr },
                    mark: {type:"line", strokeWidth:'3px'},
                    encoding: {
                        x: { field: selections_cell.GetXSelection(-1), type: "nominal", sort: selections_cell.GetSort(selections_cell.GetXSelection(-1)) },
                        y: { field: selections_cell.GetQSelection(0), type: "quantitative" },
                        color: { field: selections_cell.GetYSelection(-1), type: "nominal", sort: selections_cell.GetSort(selections_cell.GetYSelection(-1)), scale:{scheme:'set2'} },
            ...
            console.error("supportedTemplate.ANQN_Multi_Series_Line_Chart");
            break;
        ...
        case supportedTemplate.Q2_Horizon_Graph:
            if (is_horizon) {
                ...
                return new HorizonGraphTemplate(visData_arr, selections_cell, './templates/horizon graph.png');
            }
            break;
...
export function GetObjectSelections(visDataObj_arr) {
...
// Real vega-lite data
// VegaTemplate.prototype.GetVegaLite_panelchart = function (height, width) {
...
//     // 这里是为了让坐标轴在正常的范围内显示
//     console.log("GetVegaLite_panelchart", vegaLite);
//     if (vegaLite.data){
//         for (let tmp_id in vegaLite.data.values){
//             let tmp_str = vegaLite.data.values[tmp_id][vegaLite.encoding.y.field];
//             if (typeof tmp_str == 'number')
//                 break;
//             let splitArray = tmp_str.split(' > ');
//             let lastElement = splitArray[splitArray.length - 1];
//             // console.log("last", lastElement);
//             vegaLite.data.values[tmp_id][vegaLite.encoding.y.field] = lastElement;
//         }
//         for (let tmp_id in vegaLite.data.values){
//             let tmp_str = vegaLite.data.values[tmp_id][vegaLite.encoding.x.field];
//             if (typeof tmp_str == 'number')
//                 break;
//             // console.log("tmp_str", tmp_str, typeof tmp_str);
//             let splitArray = tmp_str.split(' > ');
//             let lastElement = splitArray[splitArray.length - 1];
//             // console.log("last", lastElement);
//             vegaLite.data.values[tmp_id][vegaLite.encoding.x.field] = lastElement;
//         }
//     }
...
//         // console.log("vegaLite.encoding", vegaLite.encoding, typeof vegaLite.encoding);
//         // console.log("y.sort", vegaLite.encoding.y.sort);
//         // console.log("x.sort", vegaLite.encoding.x.sort);
//         // for (let tmp_id in vegaLite.encoding.y.sort){
//         //     let tmp_str = vegaLite.encoding.y.sort[tmp_id];
//         //     let splitArray = tmp_str.split(' > ');
//         //     let lastElement = splitArray[splitArray.length - 1];
//         //     console.log("last", lastElement);
//         //     vegaLite.encoding.y.sort[tmp_id] = lastElement;
//         // }

//         // for (let tmp_id in vegaLite.encoding.x.sort){
//         //     let tmp_str = vegaLite.encoding.x.sort[tmp_id];
//         //     let splitArray = tmp_str.split(' > ');
//         //     let lastElement = splitArray[splitArray.length - 1];
//         //     console.log("last", lastElement);
//         //     vegaLite.encoding.x.sort[tmp_id] = lastElement;
//         // }
//         // console.log("y.sort.after", vegaLite.encoding.y.sort);
//         // console.log("x.sort.after", vegaLite.encoding.x.sort);
        

//         if (vegaLite.encoding.y && vegaLite.encoding.x) {
//             vegaLite.encoding.y.title = null;
//             vegaLite.encoding.x.title = null;
//             vegaLite.encoding.x.axis = {};//hp's revision
//             vegaLite.encoding.x.axis["labelAngle"] = 0;
//         }
//     }
...
VegaTemplate.prototype.GetVegaLite = function (height, width, is_panel=false) {
    let vegaLite = JSON.parse(JSON.stringify(this.vegaConfig));

    vegaLite.height = height;
    vegaLite.width = width;
    // vegaLite.padding = {"left": 30, "right": 0, "top": 0, "bottom": 30};
    vegaLite["autosize"] = {"type": "fit", "contains": "padding"};
    // NQ_PieChart: "Pie Chart",
    // NQ_RadialPlot: "Radial Plot",
    if (is_panel){
        if (this.name == supportedTemplate.NQ_PieChart || this.name == supportedTemplate.NQ_RadialPlot)
            vegaLite.config = {"legend": { "disable": false }, "axis": {"labelOverlap":"parity"}};
        else if (this.name == supportedTemplate.ANQN_Multi_Series_Line_Chart)
            vegaLite.config = {"legend": { "disable": false ,"labelLimit": 50}, "axis": {"labelOverlap":"parity"}};
        else
            vegaLite.config = {"legend": { "disable": true }, "axis": {"labelOverlap":"parity", "labelLimit": 100, "labelFlush": true}};
    }
    else
        vegaLite.config = { "axis": { "labels": false, "ticks": false, "title": null }, "legend": { "disable": true } };

    // if (is_panel && vegaLite.data){
    //     for (let tmp_id in vegaLite.data.values){
    //         let tmp_str = vegaLite.data.values[tmp_id][vegaLite.encoding.y.field];
    //         if (typeof tmp_str == 'number')
    //             break;
    //         let lastElement = tmp_str.split(' > ').at(-1);
    //         vegaLite.data.values[tmp_id][vegaLite.encoding.y.field] = lastElement;
    //     }
    //     for (let tmp_id in vegaLite.data.values){
    //         let tmp_str = vegaLite.data.values[tmp_id][vegaLite.encoding.x.field];
    ...
    if (vegaLite.encoding) {
        if (vegaLite.encoding.y && vegaLite.encoding.x) {
            ...
            vegaLite.encoding.x.title = null;
            if (is_panel){
                if (vegaLite.data && typeof vegaLite.data.values[0][vegaLite.encoding.x.field] == 'string')
                    vegaLite.encoding.x.axis = {"labelAngle":-45};//hp's revision
            }
        }
    }
    ...
    for (var i = 0; i < vegaLite.data.values.length; i++) {
        for (var key in vegaLite.data.values[i]) {
            let tmp_str = vegaLite.data.values[i][key];
            if (typeof tmp_str === 'string') {
                var modifiedString;
                if (tmp_str.length > 10)
                    // modifiedString = "..." + tmp_str.substring(tmp_str.length - 10);
                    modifiedString = "...>" + tmp_str.split(" > ").at(-1);
                else
                    modifiedString = tmp_str;
                vegaLite.data.values[i][key] = modifiedString;
            }
        }
    }
    return vegaLite;
...
VegaTemplate.prototype.ReuseTemplate = function (newMetaData_obj, newVisData_obj) {
    ...
    // if(is_density) {    // density plot 直接复用
    ...
    for (const channel in new_vegaLite.encoding) {
        if (Object.hasOwnProperty.call(new_vegaLite.encoding, channel)) {
            if (new_vegaLite.encoding[channel].field == 'density') {
                continue
            }
...
ObjTemplate.prototype.GetVegaLite = function (height, width, is_panel=false) {
    let vegaLite = JSON.parse(JSON.stringify(this.vegaConfig));
    vegaLite.height = height;
    vegaLite.width = width;
    vegaLite["autosize"] = {"type": "fit", "contains": "padding"};
    ...
    // }
    if (is_panel)
        vegaLite.config = { "axis": {"titleOpacity": 0.5, "labelOverlap":"parity", "labelLimit": 100, "labelFlush": true}, "legend": { "disable": true } };
    else
        vegaLite.config = { "axis": { "labels": false, "ticks": false, "title": null }, "legend": { "disable": true } };
...
HistogramScatterplot.prototype.GetVegaLite = function (height, width, is_panel=false) {
    let vegaLite = JSON.parse(JSON.stringify(this.vegaConfig));
    ...
    // }
    if (is_panel)
        vegaLite.config = { "axis": { "labels": true, "ticks": true, "titleOpacity": 0.5 , "labelOverlap":"parity", "labelLimit": 100, "labelFlush": true}, "legend": { "disable": true } };
    else
        vegaLite.config = { "axis": { "labels": false, "ticks": false, "title": null }, "legend": { "disable": true } };
...
HistogramHeatmap.prototype.GetVegaLite = function (height, width, is_panel=false) {
    let vegaLite = JSON.parse(JSON.stringify(this.vegaConfig));
    vegaLite["autosize"] = {"type": "fit", "contains": "padding"};
...
ParallelCoordinatePlot.prototype.GetVegaLite = function (height, width, is_panel=false) {
    ...
    mark.tooltip = true;
    // console.log("*****************", is_panel);
    if (is_panel){
        // console.log("ParallelCoordinatePlot", this.visData_arr)
        // for (let tmp of this.visData_arr){
        //     console.log("ParallelCoordinatePlottype", typeof tmp, tmp);
        // }
        // let tmp_dict = {};
        // for (let tmp_id in this.visData_arr){
        //     let modifiedDict = {};
        //     console.log("--------------------------------------");
        //     for (let key in this.visData_arr[tmp_id]) {
        //         console.log("fffffffffffff", key, "-------", this.visData_arr[tmp_id][key]);
        //         let modified_key = key.split(' > ').at(-1);
        //         if (typeof this.visData_arr[tmp_id][key] == 'string')
        //             modifiedDict[modified_key] = this.visData_arr[tmp_id][key].split(' > ').at(-1);
        //         else
        //             modifiedDict[modified_key] = this.visData_arr[tmp_id][key];
        //     }
        //     // this.visData_arr[tmp_id] = modifiedDict;
        //     tmp_dict[tmp_id] = modifiedDict;
        //     // console.log("this.visData_arr[tmp_id]", this.visData_arr[tmp_id]);
            
            
        //     // let keys = Object.keys(this.visData_arr[tmp_id]);
        //     // keys.forEach(key => {
        //     //     let modified_key = key.split(' > ').at(-1);
        //     //     this.visData_arr[tmp_id][modified_key] = this.visData_arr[tmp_id][key];
        //     //     delete this.visData_arr[tmp_id][key];
        //     // });  
        // }
        // // this.visData_arr = tmp_dict;
        // console.log("ParallelCoordinatePlot_after", tmp_dict);
        // console.log("this.GetSelections().GetQSelections()", this.GetSelections().GetQSelections());
        // let hp_test = [];
        // for (let tmp_id in this.GetSelections().GetQSelections()){
        //     hp_test.push("tmp_id");
        // }
        // console.log("hp_test", hp_test);
        // this.visData_arr = modifiedDict;
        // console.log("ParallelCoordinatePlotmodified_", this.visData_arr);
        // vegaLite["autosize"] = {"type": "fit", "contains": "padding"};

        if (this.is_horizon){
            // console.log("is_horizon", this.is_horizon);
            return {
                "height": height,
                "width": width,
                ...
                // "config": { "axis": {"title": null ,"x":{"values":hp_test}} }
                "config": { "axis": {"title": null}, "axisX":{"labelAngle": -45}}
            }
        }
        else{
            console.log("is_horizonfalse");
            return {
                "height": height,
                "width": width,
                "autosize" : {"type": "fit", "contains": "padding"},
                ...
                "layer": [
                    {
                        ...
                        "encoding": {
                            ...
                            "x": { "type": "quantitative", "field": "norm_val"}
                        }
                    },
                    {
                        "mark": { "type": "rule", "color": "#ccc" },
...
ParallelCoordinatePlot.prototype.CompileTweakedConfig = function (vegaConfig_obj) {
    this.vegaConfig = vegaConfig_obj;
    return this.GetVegaLite(300, 300, true);
    // return ;
}
...
// Real vega-lite data
HorizonGraphTemplate.prototype.GetVegaLite = function (height, width, is_panel=false) {
    ...
    let config;
    if (is_panel)
        config = { "axis": {"title": null, "labelOverlap":"parity", "labelLimit": 100, "labelFlush": true}, "axisX":{"labelAngle": -45}};
    else
        config = { "axis": { "labels": false, "ticks": false, "title": null } };
...
HorizonGraphTemplate.prototype.CompileTweakedConfig = function (vegaConfig_obj) {
    ...
    return this.GetVegaLite(200, 300, true);
}
