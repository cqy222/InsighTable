...
UnitCompiler.GetUnits = function (data_array, config) {
	// console.log("hi i am getting unit config", config)
...
UnitCompiler.GetUnitDom = function (config_obj) {
	...
	dom.addEventListener("mousemove", (event) => {
		...
		let tmp_elm = document.getElementById("unit-tooltip-element");
		if (tmp_elm != null){
			tmp_elm.childNodes[0].childNodes[0].childNodes[1].textContent = config_obj.value;
			tmp_elm.setAttribute("class", "vg-tooltip visible light-theme");
			tmp_elm.setAttribute("style", "top:" + (event.clientY + 10) + "px;" + "left:" + (event.clientX + 10) + "px;");
		}
	})
