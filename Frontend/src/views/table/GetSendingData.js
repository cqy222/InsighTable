...
export function get_data_for_transmission(top, bottom, left, right, headerRange, valueDistribution, seq2num, rlength, clength, headerDistribution, colHeader, rowHeader, removeZero=false) {
    var data = get_data_from_chosen(top, bottom, left, right, headerRange, valueDistribution, seq2num)
    
    //若数据非负则移除所有0
    if (removeZero) {   
        console.log("hihihihihi, i am removing zero!!!!")
        var hasMinus = false
        var noZero = []
        ...
        for (var i=0; i<data.length; i++) {
            ...
            if (v < 0) {    //有负数，直接结束此循环
                hasMinus = true
                console.log("i got minus !!no removing !!")
                break
            }
            else if (v > 0) {   // 不记录0
                noZero.push(data[i])
        ...
        if (!hasMinus) { // 无负数 
            data = noZero
            console.log("removing zero done!!!")
        }
    }
...
export function get_data_from_chosen(top, bottom, left, right, headerRange, valueDistribution, seq2num) {
    ...
    for (var i=top; i<=bottom; i++) {
        for (var j=left; j<=right; j++) {
            ...
            var seq = valueDistribution.get(pos)
            // console.log("hhhget_data_from_chosen", pos, "seq", seq, "seq2num.get(seq)", seq2num.get(seq))
            var value = seq2num.get(seq).value
...
function gen_json_from_data(data, chgdata, headerDistribution) {
    ...
    for (var i=0; i<data.length; i++) {
        ...
        if (j == data[i].length-1) {
            let cgd = chgdata[i][j], numcgd
            if(typeof(cgd)=="string") {
                // console.log("hihihihhiiamstring")
                numcgd = parseFloat(cgd)
            }
            else{
                // console.log("nonnononnotstring")
                numcgd = cgd
            }   
            obj["value"] = numcgd
        }
